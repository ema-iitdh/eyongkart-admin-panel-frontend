...
export default function AdminSettings() {
  ...
  const { data, isLoading, error } = useAdminById(adminId)
  console.log(data);
  ...
  if (error) return <div>Error loading admin details</div>
  if (!data) return <div>No admin data found</div>

  return (
    <div className="max-w-4xl mx-auto w-full p-4 md:p-6 lg:p-8">
      <h1 className="text-2xl md:text-3xl font-bold mb-8">Admin Settings</h1>
      {/* Back Button */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white rounded-lg shadow-lg p-6">
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="mt-1 text-base capitalize">{data.role?.replace(/_/g, " ")?.toLowerCase()}</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="mt-1 flex items-center gap-2">
              ...
              <span className="text-base">{data.isActive ? "Active" : "Inactive"}</span>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mt-1 flex items-center gap-2">
              <div className={`w-2.5 h-2.5 rounded-full ${data.isVerified ? "bg-green-500" : "bg-yellow-500"}`} />
              <span className="text-base">{data.isVerified ? "Verified" : "Unverified"}</span>
            </div>
          </div>
